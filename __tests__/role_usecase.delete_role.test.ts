...
describe("RoleUseCase - deleteRole", () => {
    ...
    it("should delete role and create audit record", async () => {
        ...
        const deletedRole = await useCase.deleteRole(user.id, roleId);
    ...
    it("should rollback and not create audit if role does not exist", async () => {
        const user = await userService.createUser({
            email: "delete_role_not_found@example.com",
            password_hash: "password123"
        });
        ...
        await expect(
            useCase.deleteRole(user.id, nonExistentRoleId)
